...
| 6 | [lab6](lab6) | No-code платформа Roboflow | классификация изображений без кода: загрузка датасета, разметка, обучение и инференс через UI; экспорт результата (отчёт в `lab6.docx`) |
...
### [lab6 — No-code платформа Roboflow](lab6)
Полный цикл «датасет → разметка → обучение → инференс» **без единой строки кода** — всё через веб-интерфейс [Roboflow](https://roboflow.com/). Контрольный пункт «другая сторона ML»: что можно сделать вообще не открывая IDE.

Что сделано на платформе:
- загрузка датасета изображений и разметка через UI;
- обучение классификатора (под капотом — AutoML с transfer learning на предобученной базовой сети);
- инференс на тестовых картинках, экспорт результатов в JSON;
- сравнение трудозатрат и степени контроля с lab5 (там та же задача классификации, но руками на PyTorch + AlexNet).

- [lab6.docx](lab6/lab6.docx) — отчёт о работе с Roboflow: скриншоты UI, описание шагов, результаты инференса.
